...
This `jupyterlab-basic` environment is designed for CI, headless setups, or scripted workflows—i.e., any non-interactive context.
...
```sh
# Clone the repo
git clone https://github.com/barstoolbluz/floxenvs && cd floxenvs/jupyterlab-basic
...
Flox v1.4+ supports environment composition, allowing you to create a customized environment that builds upon `jupyterlab-basic`. The env vars you define in `[vars]` override those hard-coded into `jupyterlab-basic`.
...
[include]
environments = [
    { remote = "barstoolbluz/jupyterlab-basic" }
]
...
- Reuse the `jupyterlab-basic` environment without modifying it
